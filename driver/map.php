...
$sql = "SELECT * FROM `data` as d  inner join students as s on d.student_ID=s.student_ID inner join parents as p on s.parent_ID=p.parent_ID  WHERE d.driver_ID='$driver_ID' AND d.status='1'";
$exe = $conn->query($sql);
$send_map = "";
while ($row = $exe->fetch_array()) {
    ...
    $send_map .= "{coords:{lat:" . $latitude . ",lng:" . $longitude . "},content:' " . $profile . "" . $name . "',iconImage:'" . $icon . "',},";
}
